...
## Using the MCP to validate knowledge before coding

**Rule: before writing code that uses any indexed library, always verify the current API against the local docs — do not rely on training data alone.**
...
Shows every library with its locally indexed version. If the version differs from what you assumed, treat your training knowledge as unreliable for that library.

### Step 2 — resolve the specific API you are about to use

```
search_docs  technology=<lib>  query=<function or concept>
```

Examples:
- `search_docs technology=supabase query="rls policy"`
- `search_docs technology=nextjs query="server actions"`
- `search_docs technology=stripe query="webhook signature verification"`
- `search_docs technology=typescript query="satisfies operator"`

### Step 3 — read the authoritative source

```
read_doc  technology=<lib>  rel_path=<path from search result>
```

Pass `query=<same query>` to get the most relevant sections first. Pass `section=<heading>` to jump directly to a subsection.

### Step 4 — cross-library lookup (when a feature spans multiple libs)

```
search_documentation  query=<concept>  technologies=[lib1, lib2, lib3]
```

Example: `search_documentation query="JWT auth" technologies=["supabase","nextjs","fastapi"]`

### Step 5 — check for recent changes

```
diff_since  since=<ISO timestamp>
```

Run this at the start of a session to see if any subscribed documentation changed since the last check. If relevant files appear, re-read them before coding.

### Mandatory after every doc read

```
submit_feedback  satisfied=<true|false>  reason=<why it helped or didn't>
```

Both fields are required. This is enforced by the server — skip it and the next tool call will remind you.

### When to use each tool

| Situation | Tool |
|---|---|
| Starting a new feature — which libs are available? | `list_supported_libraries` |
| About to call a specific function or API | `search_docs` → `read_doc` |
| Feature touches multiple libraries | `search_documentation` |
| Resuming work after a break | `diff_since` (check for doc changes) |
| Unsure if a package has docs | `resolve_observed_packages` |
| Reviewing doc quality | `feedback_stats` |
